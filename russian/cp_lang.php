<?php
$lang = array(


'no_js_warning' => 
'Javascript должен быть включен для того, чтобы эта тема функционировала должным образом. Пожалуйста, проверьте настройки браузера.',

'nav_content' => 
'Содержание',

'nav_publish' => 
'Публиковать',

'nav_edit' => 
'Редактировать',

'nav_edit_all' => 
'Все каналы',

'nav_files' => 
'Менеджер файлов',

'nav_file_manager' => 
'Файловый менеджер',

'nav_batch_upload' => 
'Массовая загрузка',

'nav_sync_files' => 
'Синхронизировать файлы',

'nav_files_short_desc' => 
'Загрузить, редактировать  удалить файл',

'nav_edit_short_desc' => 
'Изменить Записи',

'nav_design' => 
'Шаблоны',

'nav_create_group' => 
'Создать группу',

'nav_create_template' => 
'Создать шаблон',

'nav_edit_template_group' => 
'Изменить группу шаблонов',

'nav_edit_template_group_more' => 
'глядеть дальше',

'nav_templates' => 
'Шаблоны',

'nav_edit_templates' => 
'Изменить',

'nav_snippets' => 
'Сниппеты',

'nav_snippets_short_desc' => 
'Создание, изменинение, и удаление сниппетов ваших шаблонов',

'nav_sync_templates' => 
'Синхронизировать шаблоны',

'nav_sync_templates_short_desc' => 
'Синхронизовать шаблоны с связанными с ними файлами',

'nav_global_variables' => 
'Глобальные переменные',

'nav_global_variables_short_desc' => 
'Создание, изменинение, и удаление пользовательских глобальных переменных',

'nav_template_manager' => 
'Управление шаблонами',

'nav_template_manager_short_desc' => 
'Создание, изменинение, удаление и управление Шаблонами и Группами шаблонов',

'nav_template_preferences' => 
'Настройки шаблонов',

'nav_template_preferences_short_desc' => 
'Изменение настроек шаблонов',

'nav_global_preferences' => 
'Глобальные настройки',

'nav_global_preferences_short_desc' => 
'Управление шаблоном и настройками',

'nav_message_pages' => 
'Страницы сообщений',

'nav_email_notification' => 
'Email Уведомления',

'nav_email_notification_short_desc' => 
'Изменить шаблоны Email уведомлений',

'nav_user_message' => 
'Сообщения пользователя',

'nav_user_message_short_desc' => 
'Изменить шаблон пользовательских сообщений',

'nav_offline_template' => 
'Отключеные шаблоны',

'nav_offline_template_short_desc' => 
'Изменить шаблон отключенной системы',

'nav_themes' => 
'Темы',

'nav_wiki_themes' => 
'Wiki',

'nav_wiki_themes_short_desc' => 
'Изменить тему Wiki',

'nav_forum_themes' => 
'Форум',

'nav_forum_themes_short_desc' => 
'Изменить тему форума',

'nav_member_profile_templates' => 
'Шаблон профиля участника',

'nav_member_profile_templates_short_desc' => 
'Изменить шаблоны профиля учатника',

'nav_addons' => 
'Дополнения',

'nav_accessories' => 
'Аксессуары',

'nav_accessories_short_desc' => 
'Управление аксессуарами и их правами',

'nav_extensions' => 
'Расширения',

'nav_extensions_short_desc' => 
'Менеджер расширений',

'nav_plugins' => 
'Плагины',

'nav_plugins_short_desc' => 
'Установка и удаление плагинов.  Просмотр документации использования плагинов',

'nav_modules' => 
'Модули',

'nav_modules_short_desc' => 
'Управление модулем настроек',

'nav_fieldtypes' => 
'Типы полей',

'nav_fieldtypes_short_desc' => 
'Управление Fieldtypes и его настройками',

'nav_members' => 
'Участники',

'nav_view_all_members' => 
'Смотреть всё',

'nav_view_all_members_short_desc' => 
'Смотреть всех участников',

'nav_member_groups' => 
'Группы участников',

'nav_member_groups_short_desc' => 
'Создание, редактирование и удаление Групп пользователей',

'nav_ip_search' => 
'Поиск IP',

'nav_ip_search_short_desc' => 
'Поиск участников по IP адресу',

'nav_register_member' => 
'Зарегистрировать',

'nav_register_member_short_desc' => 
'Зарегистрировать нового участника',

'nav_user_banning' => 
'Забаненые',

'nav_user_banning_short_desc' => 
'Управление настройками банов и заблокированных посетителей',

'nav_activate_pending_members' => 
'Ожидающие активации',

'nav_activate_pending_members_short_desc' => 
'Модерировать ожидающих участников',

'nav_custom_member_fields' => 
'Настраиваемые поля участников',

'nav_custom_member_fields_short_desc' => 
'Создать, изменить, и удалить настраиваемые поля участников',

'nav_member_config' => 
'Настройки',

'nav_member_config_short_desc' => 
'Настройки участия',

'nav_admin' => 
'Админ',

'nav_channel_management' => 
'Админ. каналов',

'nav_channels' => 
'Каналы',

'nav_channels_short_desc' => 
'Создание, изменение, и удаление каналов',

'nav_category_management' => 
'Категории',

'nav_category_management_short_desc' => 
'Создание, изменение, и удаление категорий',

'nav_field_group_management' => 
'Настраиваемые поля',

'nav_field_group_management_short_desc' => 
'Создать, изменить, и удалить настраиваемые поля',

'nav_channel_form_settings' => 
'Каналы',

'nav_channel_form_settings_short_desc' => 
'Управление параметрами каналов',

'nav_status_group_management' => 
'Настр. статус группы',

'nav_status_group_management_short_desc' => 
'Создать, изменить, и удалить настраиваемые группы статусов',

'nav_global_channel_preferences' => 
'Глобальные настройки',

'nav_global_channel_preferences_short_desc' => 
'Настройка Глобальных Каналов',

'nav_admin_content' => 
'Админ. контента',

'nav_file_manager_short_desc' => 
'Настройки загрузки файлов',

'nav_file_upload_preferences' => 
'Настройки загрузки файлов',

'nav_file_upload_preferences_short_desc' => 
'Создать, редактировать или удалить настройки загрузки файлов',

'nav_file_watermark_preferences' => 
'Настройки водяного знака',

'nav_file_watermark_preferences_short_desc' => 
'Настройки водяного знака',

'nav_default_html_buttons' => 
'Стандартные HTML кнопки',

'nav_default_html_buttons_short_desc' => 
'Создать, изменить, и удалить HTML кнопки',

'nav_security_and_privacy' => 
'Права и безопасность',

'nav_admin_system' => 
'Админ. системы',

'nav_general_configuration' => 
'Основные настройки',

'nav_general_configuration_short_desc' => 
'Управление основными настройками',

'nav_email_configuration' => 
'Настройки Email',

'nav_email_configuration_short_desc' => 
'Управление настройками Email',

'nav_localization_settings' => 
'Настройки локал.',

'nav_localization_settings_short_desc' => 
'Управление настройками локализации',

'nav_database_settings' => 
'Настройки базы данных',

'nav_database_settings_short_desc' => 
'Управление настройками базы данных',

'nav_output_debugging_preferences' => 
'Вывод и отладка',

'nav_output_debugging_preferences_short_desc' => 
'Настройки вывода и отладки',

'nav_security_session_preferences' => 
'Сессии и безопасность',

'nav_security_session_preferences_short_desc' => 
'Настройки безопасности и сессий',

'nav_captcha_preferences' => 
'Параметры CAPTCHA',

'nav_captcha_preferences_short_desc' => 
'Управление параметрами CAPTCHA',

'nav_throttling_configuration' => 
'Настройки снижения активности посетителей',

'nav_throttling_configuration_short_desc' => 
'Конфигурация снижения активности посетителей',

'nav_tracking_preferences' => 
'Настройка посещениями',

'nav_tracking_preferences_short_desc' => 
'Управление настройками посещений',

'nav_cookie_settings' => 
'Настройки куков',

'nav_cookie_settings_short_desc' => 
'Управление настройками куков',

'nav_image_resizing_preferences' => 
'Настройки изменения изображения',

'nav_image_resizing_preferences_short_desc' => 
'Управление настройками  изменения изображения',

'nav_emoticon_preferences' => 
'Настройки смайликов',

'nav_emoticon_preferences_short_desc' => 
'Управление настройками смайликов',

'nav_word_censoring' => 
'Цензура слов',

'nav_word_censoring_short_desc' => 
'Настройки Цензуры слов',

'nav_search_log_configuration' => 
'Конфигурация журнала регистрации поисковых запросов',

'nav_search_log_configuration_short_desc' => 
'Управление настройками журнала регистрации поисковых запросов',

'nav_tools' => 
'Инструменты',

'nav_tools_communicate' => 
'Почта',

'nav_tools_communicate_short_desc' => 
'Отправка писем',

'nav_tools_utilities' => 
'Утилиты',

'nav_translation_tool' => 
'Утилиты перевода',

'nav_translation_tool_short_desc' => 
'Утилиты перевода',

'nav_import_utilities' => 
'Утилиты импорта',

'nav_import_utilities_short_desc' => 
'Импорт данных',

'nav_php_info' => 
'Информация о PHP',

'nav_php_info_short_desc' => 
'Подробная PHP информация',

'nav_config_editor' => 
'Редактор сonfig',

'nav_config_editor_short_desc' => 
'Редактор конфигурации',

'nav_tools_data' => 
'Данные',

'nav_sql_manager' => 
'SQL Менеджер',

'nav_sql_manager_short_desc' => 
'Утилиты управления базой данных',

'nav_clear_caching' => 
'Очистка кеша',

'nav_clear_caching_short_desc' => 
'Очистить кешированные данные',

'nav_search_and_replace' => 
'Поиск и замена',

'nav_search_and_replace_short_desc' => 
'Поиск и замена',

'nav_recount_stats' => 
'Пересчитать статистику',

'nav_recount_stats_short_desc' => 
'Пересчитать и обновить статистику',

'nav_tools_logs' => 
'Журналы',

'nav_view_cp_log' => 
'Журнал панели управления',

'nav_view_cp_log_short_desc' => 
'Просмотр и очистка журнала панели управления',

'nav_view_search_log' => 
'Журнал поиска',

'nav_view_search_log_short_desc' => 
'Просмотр и очистка журнала регистрации поисковых запросов',

'nav_view_throttle_log' => 
'Журнал снижения активности посетителей',

'nav_view_throttle_log_short_desc' => 
'Просмотр и очистка снижения активности посетителей',

'nav_view_email_log' => 
'Журнал Email консоли',

'nav_view_email_log_short_desc' => 
'Просмотр и очистка Email журнала',

'nav_view_developer_log' => 
'Журнал разработчика',

'nav_view_developer_log_short_desc' => 
'Просмотр и управление журналом разработчика',

'nav_help' => 
'Помощь',

'nav_overview' => 
'Обзор',

'nav_add_tab' => 
'+ Добавить',

'session_expiring' => 
'Ваша сессия истекла. Пожалуйста, войдите в систему, чтобы избежать потери данных',

'loading' => 
'Загрузка',

'show_hide' => 
'Показать/Скрыть все',

'new_tab' => 
'Новая закладка',

'new_version_available' => 
'Доступна новая версия ExpressionEngine - %s',

'consult_user_guide' => 
'См. Руководство',

'powered_by' => 
'Паботает под управлением',

'control_panel' => 
'Панель управления',

'main_menu' => 
'Главная',

'username' => 
'Пользователь',

'password' => 
'Пароль',

'logged_back_in' => 
'Теперь вы снова войдете в систему!',

'my_account' => 
'Аккаунт',

'myaccount' => 
'Аккаунт',

'user_account' => 
'Аккаунт участника',

'user_guide' => 
'Руководство',

'logout_confirm' => 
'Вы уверены что хотите выйти?',

'logout' => 
'Выход',

'session_timeout' => 
'Ваша сессия истекает в ближайшее время из-за неактивности',

'login' => 
'Вход',

'copyright' => 
'Авторское право',

'rights_reserved' => 
'Все права защищены',

'page_rendered' => 
'Скрипт выполнен за %x сек.',

'queries_executed' => 
'с использованием %x SQL-запросов',

'build' => 
'Подверсия:',

'forgot_password' => 
'Забыли пароль?',

'e_no_css' => 
'Ошибка обращения к CSS-файлу.',

'unauthorized' => 
'Ограничение',

'unauthorized_access' => 
'У Вас нет прав доступа к этой странице',

'be_careful' => 
'ОСТОРОЖНО!',

'action_can_not_be_undone' => 
'ЭТО ДЕЙСТВИЕ НЕ МОЖЕТ БЫТЬ ОТМЕНЕНО',

'captcha_explanation' => 
'СAPTCHA &#8211; это изображение защитного кода, который должен ввести участник.  Более подробную информацию смотрите в Руководстве.',

'back' => 
'Вернуться',

'no' => 
'Нет',

'yes' => 
'Да',

'all' => 
'Все',

'any' => 
'Все',

'required_fields' => 
'Обязательные для заполнения поля',

'forgotten_password' => 
'Восстановление пароля',

'submit_email_address' => 
'Введите Ваш адрес E-mail',

'click_to_edit' => 
'Нажмите для изменения',

'notepad' => 
'Блокнот',

'notepad_no_content' => 
'Нет записей в блокноте. Нажмите для создания одной',

'edit_quicklinks' => 
'Редактировать Быстрые ссылки',

'quick_links' => 
'Быстрые ссылки',

'quicklinks_manager' => 
'Управление быстрыми ссылками',

'hide_sidebar' => 
'Скрыть боковую панель',

'reveal_sidebar' => 
'Вернуть боковую панель',

'submit' => 
'Отправить',

'preview' => 
'Предпросмотр',

'update' => 
'Обновить',

'edit' => 
'Редактировать',

'delete' => 
'Удалить',

'save_changes' => 
'Сохранить',

'save' => 
'Сохранить',

'sync' => 
'Синхронизировать',

'cancel' => 
'Отмена',

'done' => 
'Выход',

'clear_logs' => 
'Очистить файлы журналов',

'member_id' => 
'ID участника',

'ip_address' => 
'IP-адрес',

'date' => 
'Дата',

'action' => 
'Действие',

'member_logged_in' => 
'В системе',

'member_logged_out' => 
'Вне системы',

'cleared_logs' => 
'Очищенные файлы журналов',

'close' => 
'Закрыть',

'create' => 
'Создать',

'create_and_edit' => 
'Создать или изменить',

'modify' => 
'Изменить',

'or_delete' => 
'или удалить',

'view' => 
'Просмотр',

'switch' => 
'Сменить',

'entry' => 
'Запись',

'template' => 
'Шаблон',

'template_group' => 
'Группа шаблонов',

'page' => 
'Страница',

'channel' => 
'Канал',

'site' => 
'Сайт',

'most_recent_entry' => 
'Последние записи',

'most_recent_comment' => 
'Последний комментарий',

'recent_entries' => 
'Последние записи ',

'recent_comments' => 
'Последние комментарии',

'ee_wiki' => 
'EE Wiki',

'status' => 
'Статус',

'select_all' => 
'Выбрать/Отменить все',

'name' => 
'Имя',

'sites' => 
'Сайты',

'view_site' => 
'Посмотреть сайт',

'edit_sites' => 
'Редактировать сайты',

'edit_channels' => 
'Изменить каналы',

'site_id' => 
'ID сайта',

'site_specific_data' => 
'Эти данные не для всех сайтов.  Если Вы хотите выбрать или изменить данные другого сайта, используйте закладку меню «Сайты» для переключения на этот сайт.',

'sig_img_path' => 
'Путь на сервере к директории для загрузки на сервер изображений для подписей',

'avatar_path' => 
'Путь на сервере к директории с аватарами',

'photo_path' => 
'Путь на сервере к директории с фотографиями',

'prv_msg_upload_path' => 
'Путь на сервере к директории загрузки вложений',

'not_writable_path' => 
'Указанный Вами путь защищен от записи.  Убедитесь, что для директории установлены права доступа 777.',

'videos' => 
'Видео',

'community_tutorials' => 
'Обучения',

'community_resources' => 
'Ресурсы сообщества',

'support' => 
'Техническая Поддержка',

'installing_ee' => 
'Установка ExpressionEngine',

'introduction_to_templates' => 
'Вступление и Шаблоны',

'channels_custom_fields' => 
'Каналы и Настраиваемые поля',

'channel_template_relationship' => 
'Связи каналов в шаблонах',

'building_ee_site_01' => 
'Делаем сайты на ExpressionEngine - Часть 01',

'designing_ee_architecture' => 
'Проектирование и архитектура ExpressionEngine',

'troubleshooting_file_uploads' => 
'Устранение неполадок с загрузкой файлов',

'ee_cp_overview' => 
'Обзор панели управления ExpressionEngine',

'ee_seach_bookmarklet' => 
'ExpressionEngine Поиск закладок',

'wiki' => 
'Wiki',

'support_forums' => 
'Форум поддержки',

'knowledge_base' => 
'База знаний',

'documentation' => 
'Документация',

'my_downloads' => 
'Мои загрузки',

'train_ee' => 
'Train-EE',

'ee_screencasts' => 
'EE Screencasts',

'ee_insider' => 
'EE Insider',

'devot_ee' => 
'Devot-EE',

'ee_podcast' => 
'EE Podcast',

'translate' => 
'Обновить',

''=>''
);

// End of File
<?php
$lang = array(


'search_module_name' => 
'Поиск',

'search_module_description' => 
'Модуль поиска',

'search_no_stopwords' => 
'В Вашем поисковом запросе содержатся не используемые при поиске слова.',

'search_exact_word' => 
'Искать точное совпадение слова',

'search_any_words' => 
'Искать любое из слов',

'search_all_words' => 
'Искать все перечисленные слова',

'search_keywords' => 
'Ключевые слова',

'search_total_results' => 
'Всего результатов поиска:',

'search_channel' => 
'Канал',

'search_excerpt' => 
'Какое поле следует использовать для цитат, отображаемых на страницах результатов поиска?',

'search_author' => 
'Автор',

'search_any_channel' => 
'Любой канал',

'search_total_comments' => 
'Всего комментариев',

'search_recent_comment_date' => 
'Последний комментарий',

'search_by_keyword' => 
'Поиск по ключевым словам',

'search_in_titles' => 
'Искать в заголовках',

'search_entries' => 
'Искать в заголовках и записях',

'search_everywhere' => 
'Искать в заголовках, записях и комментариях',

'search_exact_phrase_match' => 
'Точное совпадение фразы',

'search_by_member_name' => 
'Поиск по имени',

'search_by_member_group' => 
'Поиск по группе',

'switch_to_group' => 
'Включить поиск по группам',

'switch_to_name' => 
'Включить поиск по имени',

'search_all_groups' => 
'Искать во всех группах',

'search_exact_name_match' => 
'Точное совпадение имени',

'search_channels' => 
'Каналы',

'search_categories' => 
'Категории',

'search_also_search_comments' => 
'Искать также в комментариях',

'search_any_date' => 
'За все время',

'search_today_and' => 
'Сегодня и ...',

'search_this_week_and' => 
'На этой неделе и ...',

'search_one_month_ago_and' => 
'1 месяц назад и ...',

'search_three_months_ago_and' => 
'3 месяца назад и ...',

'search_six_months_ago_and' => 
'6 месяцев назад и ...',

'search_one_year_ago_and' => 
'1 год назад и ...',

'search_newer' => 
'позже',

'search_older' => 
'раньше',

'search_sort_results_by' => 
'Сортировка результатов по признаку:',

'search_date' => 
'Дата',

'search_title' => 
'Заголовок',

'search_most_comments' => 
'Кол-во комментариев',

'search_recent_comment' => 
'Последний комментарий',

'search_descending' => 
'по убыванию',

'search_ascending' => 
'по возрастанию',

'search_entries_from' => 
'Искать в записях, опубликованных:',

'search_any_category' => 
'Любая категория',

'search_engine' => 
'Поиск',

'search' => 
'Найти',

'search_path_error' => 
'Ошибка указания пути.  Обратитесь к системному администратору.',

'search_not_allowed' => 
'Доступ к поиску закрыт.',

'search_no_keywords' => 
'Не был задан поисковый запрос',

'search_min_length' => 
'В запросе могут использоваться цепочки символов (слова), состоящие минимум из %x знаков',

'search_result_heading' => 
'Результаты поиска:',

'search_no_result' => 
'В результате поиска по Вашему запросу совпадений не обнаружено.',

'search_time_not_expired' => 
'Поиск может осуществляться не чаще 1 раза в %x секунд.',

'search_results' => 
'Результаты поиска',

'search_type' => 
'Тип поиска',

'none_of_these_words' => 
'Ни одно из этих слов',

'search_in' => 
'Искать в',

'search_in_channels' => 
'Искать в каналах',

'any_field' => 
'Любое поле',

'search_in_articles' => 
'Искать в записях',

'search_in_titles_articles' => 
'Искать в заголовках и записях',

'search_content_from' => 
'Искать в записях, опубликованных:',

'date_since' => 
'С',

'today' => 
'Сегодня',

'past_week' => 
'За последнюю неделю',

'past_month' => 
'За последние 30 дней',

'past_six_months' => 
'За последние полгода',

'past_year' => 
'За последние 365 дней',

'categories' => 
'Категории',

'any_category' => 
'Любая категория',

'rebuild_search_index' => 
'Перестроить поисковый индекс',

'searched_for' => 
'Поиск по:',

''=>''
);

/* End of file search_lang.php */
/* Location: ./system/expressionengine/language/russian/search_lang.php */